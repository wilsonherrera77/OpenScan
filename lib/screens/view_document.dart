  ...
  Future<dynamic> createImage({
    ...
    if (image != null || galleryImages != null) {
      if (!(quickScan || fromGallery)) {
        ...
        imageFilePath = await scannerService.cropAndCompress(image!.path, quality: 85);
        imageFilePath ??= image!.path; // Fallback to original if crop failed
      }
        ...
        children: <Widget>[
          ...
          ListTile(
            ...
            onTap: () async {
              ...
              // Show enqueue result to user via SnackBar
              if (!shareEnqueued) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('⚠️ Error al encolar: ${shareError ?? "Unknown"}'),
                    backgroundColor: Colors.orange,
              ...
              await Share.shareXFiles([XFile(pdfPath)], text: 'Compartir PDF');
              Navigator.pop(context);
            },
          ),
          ListTile(
            ...
            onTap: () async {
              ...
              if (savedDirectory != null) {
                try {
                  ...
                  // Clean filename (same as in file_operations.dart)
                  String cleanFileName = fileName.replaceAll('-', '').replaceAll('.', '').replaceAll(':', '');
                  final pdfPath = '$savedDirectory/$cleanFileName.pdf';
                  final pdfFile = File(pdfPath);
          ...
          ListTile(
            ...
            onTap: () async {
              ...
              final imagePaths = (enableSelect) ? selectedImagesPath : imageFilesPath;
              await Share.shareXFiles(imagePaths.map((path) => XFile(path)).toList(), text: 'Compartir imágenes');
              Navigator.pop(context);
            },
          ),
          // (enableSelect)
